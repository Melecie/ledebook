...
for page in allPages:
    ...
    if pageinfo == []:
        ...
        imageinfo = "none"
        for img in images:
            # exclude other media types including svg due to their frequent use in maintenance/template images that are usually first
...
for x in range(0, articleNo): # writing lede text
    ...
    if imgList[x] != "none":
        outputHTML += "<img src='" + imgList[x] + "' height='128px' alt='" + imgNameList[x] + "' style='float:right'>"
...
outputHTML += "<br><i>Ledebook generator by Melecie, contributions by various users from various English Wikipedia pages licensed under <a href='https://creativecommons.org/licenses/by-sa/3.0/'>Creative Commons Attribution-ShareAlike License 3.0</a></i><br><ul>"
...
if noOfImages > 0:
    ...
    for x in range(0, articleNo): # writing credit links to images
        ...
        if imgInfo != "none":
            ...
            outputHTML += "<li>"
            try: outputHTML += "<b>" + titleList[x] + ":</b> <a href='https://commons.wikimedia.org/wiki/File:" + imgNameList[x] + "'>" + imgInfo["ObjectName"]["value"]+ "</a>"
            except: imgErrorNo += 1
            try: outputHTML += " | <b>Credit:</b> " + imgInfo["Credit"]["value"]
            except: imgErrorNo += 1
            try: outputHTML += " | <b>Artist:</b> " + imgInfo["Artist"]["value"]
            except: imgErrorNo += 1
            try: outputHTML += " | <b>License:</b> <a href='" + imgInfo["LicenseUrl"]["value"] + "'>" + imgInfo["UsageTerms"]["value"]+ "</a>"
            except: outputHTML += " | " + imgInfo["UsageTerms"]["value"]
            outputHTML += "</li>"
